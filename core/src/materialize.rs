...
pub use mcp::{McpPlan, PlannedMcpWrite, ReconcileReport, apply_mcp, plan_mcp, reconcile_mcp};
pub use plan::{
    MaterializePlan, PlannedDelete, PlannedWrite, apply, plan_materialize, reconcile_items,
};
    ...
    for agent in adapters::registry() {
        for &scope in &[Scope::Global, Scope::Project] {
            if agent.supported_scopes().contains(&scope) && !agent.experimental_read_only() {
                match plan_materialize(agent.as_ref(), store, scope, project_root) {
                    Ok(plan) if !plan.is_empty() => {
                        match apply(agent.as_ref(), &plan, scope, project_root) {
                            Ok(_) => applied += 1,
            ...
            match plan_mcp(agent.as_ref(), store, scope, project_root) {
                Ok(plan) if !plan.is_empty() => {
                    match apply_mcp(agent.as_ref(), store, &plan, scope, project_root) {
                        Ok(_) => applied += 1,
                        Err(e) => notes.push(format!(
