...
impl Agent for Gemini {
    ...
    fn root(&self, scope: Scope, project_root: &Path) -> Option<PathBuf> {
        match scope {
            ...
            // global scope is skipped rather than writing a `.gemini/GEMINI.md`
            // into the current working directory.
            Scope::Global => Some(crate::platform::home_dir()?.join(".gemini")),
    ...
    fn discover_existing(
        ...
        discover_single_file(&root.join("GEMINI.md"), scope)
    }
