...
use crate::security::{path_guard, secret_scan};
...
fn guarded_mcp_path(store_root: &std::path::Path, name: &str) -> Result<PathBuf> {
    let path = mcp_path(store_root, name);
    let relative = path
...
impl Store {
    pub fn save_mcp_server(&self, server: &McpServer) -> Result<()> {
        self.check_schema()?;
        server.validate()?;
        let target = guarded_mcp_path(&self.root, &server.name)?;
        ...
        let toml_str = toml::to_string_pretty(server).map_err(|e| Error::Toml {
            path: target.clone(),
            message: e.to_string(),
        })?;
        ...
        // run `frontmatter_limits::validate_raw`: that check exists to bound
        // YAML parser expansion before a YAML parse, and nothing here is ever
        // parsed as YAML.
        ...
        std::fs::write(&target, toml_str).map_err(|source| Error::Io {
            path: target,
            source,
        })
    }
    ...
    pub fn load_mcp_server(&self, name: &str) -> Result<McpServer> {
        self.check_schema()?;
        validate_name(name)?;
        let target = guarded_mcp_path(&self.root, name)?;
