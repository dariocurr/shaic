...
impl App {
    pub fn new() -> crate::Result<Self> {
        ...
        let has_store = Store::open(Store::default_path()).is_ok();
        let mut app = App {
            screen: if has_store {
                Screen::Dashboard
            } else {
    ...
    pub fn push(&mut self) {
        self.message = match Store::open(Store::default_path()).and_then(|s| s.push(false)) {
            Ok(r) if r.committed => format!("pushed: {}", r.summary.unwrap_or_default()),
    ...
    pub fn pull(&mut self) {
        self.message = match Store::open(Store::default_path()).and_then(|s| s.pull()) {
    ...
    pub fn run_wizard(&mut self) {
        ...
        if let Err(e) = shaic_core::store::git::ls_remote(&url) {
            self.wizard.status = format!("could not reach {url}: {e}");
            return;
        }
        if let Err(e) = Store::init(Store::default_path(), Some(&url)) {
        ...
        self.message = match Config::load() {
            Ok(mut config) => {
                ...
                format!("store ready (remote: {url})")
            }
            Err(e) => format!(
                "store ready (remote: {url}), but config is corrupted and was left untouched: {e}"
            ),
        };
    ...
    pub fn confirm_add_name(&mut self) -> Option<PendingAction> {
        let name = self.browser.name_input.take()?;
    ...
    pub fn finish_edit(
        ...
        let result = Store::open(Store::default_path()).and_then(|store| {
    ...
    pub fn open_diff_preview(&mut self, agent: AgentId, scope: Scope, is_mcp: bool) {
        ...
        let plan = if is_mcp {
            materialize::plan_mcp(agent_impl.as_ref(), &store, scope, &self.project_root)
                .map(PreviewPlan::Mcp)
        } else {
            materialize::plan_materialize(agent_impl.as_ref(), &store, scope, &self.project_root)
    ...
    pub fn apply_diff_preview(&mut self) {
        ...
        let agent_impl = adapters::by_id(diff.agent);
        // Both branches pull this agent's own on-disk content into the store
        // before pushing the store's state back out to it — same reason
        // `sync` does: something added or edited directly in this agent's
        // config should reach every other agent, not just sit here or get
        // clobbered by the store's stale copy. A pull failure is not fatal to
        // the apply itself, so it only ever reduces the reported count.
        let mut rejections = Vec::new();
        let result = match &diff.plan {
            PreviewPlan::Base(plan) => {
                ...
                materialize::apply(agent_impl.as_ref(), plan, diff.scope, &self.project_root).map(
                    |applied| {
                        let changed = applied
                            .iter()
            ...
            PreviewPlan::Mcp(plan) => Store::open(Store::default_path()).and_then(|store| {
                let mut pulled = 0;
                if let Ok(report) = materialize::reconcile_mcp(
                    agent_impl.as_ref(),
                    &store,
                    diff.scope,
                    &self.project_root,
        ...
        let rejected_suffix = if self.pull_rejections.is_empty() {
            ...
            let names: Vec<_> = self
                ...
                .collect();
            format!(" — could not pull: {}", names.join(", "))
        };
        match result {
            Ok((changed, 0)) => {
                self.message = format!("applied {changed} change(s){rejected_suffix}")
            }
            Ok((changed, pulled)) => {
                self.message = format!(
                    "applied {changed} change(s), pulled {pulled} from this agent{rejected_suffix}"
                )
            }
    ...
    pub fn open_agent_detail(&mut self) {
        ...
        let sub_rows: Vec<AgentDetailSubRow> = row
            .sub_rows
            .iter()
            .map(|sub| self.build_detail_sub_row(agent_impl.as_ref(), sub, &store))
            .collect();
    ...
    fn build_detail_sub_row(
        ...
        let content_glyph = sub.content_glyph.is_some().then(|| {
            ...
            lines.push(format!(
                "location: {}",
                agent_impl.root(scope, &self.project_root).display()
            ));
