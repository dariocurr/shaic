...
pub fn draw(frame: &mut Frame, app: &App) {
    ...
    let skipped = match &diff.plan {
        PreviewPlan::Base(plan) => {
            for write in plan.changed_writes() {
                lines.push(change_line(write.action, &write.relative_path.display()));
            }
        ...
        PreviewPlan::Mcp(plan) => {
            for write in plan.changed_writes() {
                lines.push(change_line(write.action, &write.name));
            }
    ...
    frame.render_widget(
        screens::footer(&app.message, "   [a=apply  ?=help  Esc=cancel]"),
        chunks[3],
    );
}

/// One pending create/update, rendered like a diff hunk line (`+`/`~`).
/// `WriteAction::NoOp` never reaches here — both callers iterate
/// `changed_writes()`.
fn change_line(action: WriteAction, target: &dyn std::fmt::Display) -> ListItem<'static> {
    let (symbol, label, color) = match action {
        WriteAction::Create => ("+", "create", theme::SUCCESS),
        WriteAction::Update => ("~", "update", theme::WARNING),
        WriteAction::NoOp => unreachable!(),
    };
